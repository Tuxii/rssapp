...
exports.find = function(table, fields, all, condition){
	...
	while(results.isValidRow()){
		var obj = {};
		obj[results.field] = results.fieldName;
		Ti.API.info(JSON.stringify(obj));
		Ti.API.info(results.field);
		data.push({url : results.fieldByName('url'), title: results.fieldByName('title'), icon: results.fieldByName('icon')});
		results.next();
	};
	return data;
};
